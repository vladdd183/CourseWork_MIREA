...
<p align="center">
  <img src="docs/examples/page-01.png" alt="Титульный лист" width="180">
  <img src="docs/examples/page-02.png" alt="Оглавление" width="180">
  <img src="docs/examples/page-05.png" alt="Глава с таблицей" width="180">
  <img src="docs/examples/page-06.png" alt="Формулы" width="180">
  <img src="docs/examples/page-10.png" alt="Листинг кода" width="180">
  <img src="docs/examples/page-11.png" alt="Таблица + график" width="180">
</p>

<p align="center"><em>Титульный лист &bull; Оглавление &bull; Таблица сравнения &bull; Формулы &bull; Листинг кода &bull; График</em></p>
...
📄 Результат: `main.pdf` — готовый к печати документ:

<p align="center">
  <img src="docs/examples/page-01.png" alt="Титульный лист" width="300">
  &nbsp;&nbsp;&nbsp;
  <img src="docs/examples/page-02.png" alt="Оглавление" width="300">
</p>
<p align="center"><em>Титульный лист и автосгенерированное оглавление — из коробки</em></p>
...
<p align="center">
  <img src="docs/examples/page-05.png" alt="Таблица сравнения с зеброй" width="500">
</p>
<p align="center"><em>Таблица 1.1 с чередованием строк, цитированиями [3] и [4-7]</em></p>
...
<p align="center">
  <img src="docs/examples/page-10.png" alt="Листинг кода с подсветкой синтаксиса" width="500">
</p>
<p align="center"><em>Листинг 3.1 — подсветка синтаксиса Go, нумерация строк, фиолетовая полоса</em></p>
...
Формулы нумеруются автоматически в секционном формате `(Глава.Номер)`:

```typst
// Просто используйте блочный math — нумерация (N.M) появится автоматически
$ "EMA"_t = alpha dot x_t + (1 - alpha) dot "EMA"_(t-1) $

$ z = (x - mu) / sigma, quad "где" mu = 1/n sum_(i=1)^n x_i $

$ D_M (arrow(x)) = sqrt((arrow(x) - arrow(mu))^T Sigma^(-1) (arrow(x) - arrow(mu))) $
```

<p align="center">
  <img src="docs/examples/page-06.png" alt="Математические формулы с секционной нумерацией" width="500">
</p>
<p align="center"><em>Формулы (1.1), (1.2), (1.3) — LaTeX-качество, автонумерация</em></p>
...
Шаблон включает обёртки для [CeTZ](https://github.com/cetz-package/cetz) и [CeTZ-Plot](https://github.com/cetz-package/cetz-plot) — библиотек для рисования **векторной** графики прямо в Typst. Никаких растровых картинок — всё генерируется при компиляции.

> 🎨 **Галерея примеров**: [diagrams.janosh.dev](https://diagrams.janosh.dev/) — 130+ научных диаграмм на CeTZ

<p align="center">
  <img src="docs/examples/page-11.png" alt="Таблица результатов + график в документе" width="500">
</p>
<p align="center"><em>Как это выглядит в документе: таблица + график на одной странице</em></p>

### 📈 График функции

Многокривая с сеткой, легендой и математическими подписями осей:

<p align="center">
  <img src="docs/examples/plot.png" alt="График: затухающая синусоида с огибающей" width="700">
</p>
...
#figure(
  ...
  caption: "Затухающие колебания с двумя гармониками",
)
```
...
Секторная диаграмма с подписями-выносками и процентами:

<p align="center">
  <img src="docs/examples/piechart.png" alt="Круговая диаграмма: технологии контейнеризации" width="600">
</p>
...
Горизонтальный бар-чарт с градиентом и подписями по кварталам:

<p align="center">
  <img src="docs/examples/barchart.png" alt="Столбчатая диаграмма: динамика запросов к API" width="700">
</p>
...
#figure(
  ...
  caption: "Динамика запросов к API по кварталам",
)
```
...
Сложная блок-схема с ветвлениями, цветными блоками по типу (терминатор / ввод-вывод / условие / процесс) и обратными связями:

<p align="center">
  <img src="docs/examples/flowchart.png" alt="Блок-схема: алгоритм аутентификации" width="450">
</p>
...
#figure(
  gost-canvas({
    ...
    flowchart-arrow((0, -0.45), (0, -1.15))

    // Ввод данных (параллелограмм)
    flowchart-block(cetz, (0, -1.6), [Ввод логина и пароля])
    flowchart-arrow((0, -2.05), (0, -2.75))

    // Условие: формат валиден?
    flowchart-block(cetz, (0, -3.55), [Формат валиден?], shape: "diamond")
    flowchart-arrow((0, -4.35), (0, -5.15))  // Да — вниз

    // Ветка «Нет» → вправо → ошибка → назад к вводу
    flowchart-arrow((2.0, -3.55), (4.2, -3.55))
    flowchart-block(cetz, (6.3, -3.55), [Ошибка формата])

    // Запрос к БД
    flowchart-block(cetz, (0, -5.6), [Запрос к БД: поиск пользователя])
    flowchart-arrow((0, -6.05), (0, -6.75))

    // Условие: пользователь найден?
    flowchart-block(cetz, (0, -7.55), [Пользователь найден?], shape: "diamond")
    flowchart-arrow((0, -8.35), (0, -9.15))  // Да

    // Проверка пароля
    flowchart-block(cetz, (0, -9.6), [Хэширование и проверка пароля])
    flowchart-arrow((0, -10.05), (0, -10.75))

    // Условие: пароль верный?
    flowchart-block(cetz, (0, -11.55), [Пароль верный?], shape: "diamond")
    flowchart-arrow((0, -12.35), (0, -13.15))  // Да

    // Генерация токена → вывод → конец
    flowchart-block(cetz, (0, -13.6), [Генерация JWT-токена])
    flowchart-arrow((0, -14.05), (0, -14.75))
    flowchart-block(cetz, (0, -15.2), [Вернуть токен клиенту])
    flowchart-arrow((0, -15.65), (0, -16.35))
    flowchart-block(cetz, (0, -16.8), [Конец], shape: "stadium")
  }),
  caption: "Блок-схема алгоритма аутентификации пользователя",
)
```
